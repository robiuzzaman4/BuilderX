...
// === PATCH: Publish/Unpublish platform ===
export async function PATCH(
  ...
  try {
    ...
    // === check if platform has components before publishing ===
    if (isPublished && platform.pageStructure.length === 0) {
      return Response.json(
        {
          success: false,
          message: "Cannot publish an empty platform. Add components first.",
        },
        { status: 400 }
      );
    }
    ...
    platform.isPublished = isPublished;

    // === set publishedAt date if publishing for the first time ===
    if (isPublished && !platform.publishedAt) {
